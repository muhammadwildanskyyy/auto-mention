import { findAutomation } from "@/src/actions/automations/queries";
import {
  createChatHistory,
  getChatHistory,
  ...
  matchKeyword,
  trackResponses,
} from "@/src/actions/webhook/queries";
import { sendDM, sendPrivateMessage } from "@/src/lib/fetch";
                  ...
                  data: {
                    dms: {
                      create: {
                        receiver:
                          webhook_payload.entry[0].messaging[0].sender.id,
                        senderId: webhook_payload.entry[0].id,
                        message:
                          webhook_payload.entry[0].messaging[0].message.text,
                      },
                    },
        ...
        if (automation && automations_post && automation.trigger) {
          ...
          if (automation.listener) {
            ...
            if (automation.listener.listener === "MESSAGE") {
              ...
              console.log("dm sent");
              console.log(4);
              console.log("status", direct_message.status);
